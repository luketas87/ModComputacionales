...
namespace DAL
{
    public class CuentaUsuarioDAL
    {
        ...
        {
            DAO mDAObject = new DAO();
            DataSet mDs = mDAObject.ExecuteDataSet("select Cuenta_usuario_id, Cuenta_usuario_username, Cuenta_usuario_password, Cuenta_usuario_intentos_login,  cuenta_usuario_activa, cuenta_fecha_alta, year(Cuenta_fecha_alta) as anio, month(Cuenta_fecha_alta) as mes, day(Cuenta_fecha_alta) as dia from cuenta_usuario where cuenta_usuario_username = '" + pUsername + "' ");
            if (mDs.Tables.Count > 0 && mDs.Tables[0].Rows.Count > 0)
            {
        ...
        public static List<CuentaUsuario> Listar()
        {
            ...
            mDs = mDAObject.ExecuteDataSet("select Cuenta_usuario_id, Cuenta_usuario_username, Cuenta_usuario_password, Cuenta_usuario_intentos_login,  cuenta_usuario_activa, cuenta_fecha_alta, year(Cuenta_fecha_alta) as anio, month(Cuenta_fecha_alta) as mes, day(Cuenta_fecha_alta) as dia from cuenta_usuario where cuenta_usuario_activa = 1");
        ...
        public static List<CuentaUsuario> ObtenerUsuarios(Familia pFamilia)
        {
            ...
            DataSet mDs = mDAObject.ExecuteDataSet("select t2.cuenta_usuario_id, t2.cuenta_usuario_username, t2.cuenta_usuario_password, t2.cuenta_usuario_intentos_login, year(t2.Cuenta_fecha_alta) as anio, month(t2.Cuenta_fecha_alta) as mes, day(t2.Cuenta_fecha_alta) as dia from cuenta_usuario_familia t1 inner join cuenta_usuario t2 on t1.cuenta_usuario_id = t2.cuenta_usuario_id where t1.familia_id = " + pFamilia.familia_id);
            if (mDs.Tables.Count > 0 && mDs.Tables[0].Rows.Count > 0)
            {
